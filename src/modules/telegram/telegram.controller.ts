...
export class TelegramController extends BaseController implements ITelegramController {
	...
	async sendConsultationController(req: Request, res: Response, next: NextFunction): Promise<void> {
		try {
			...
			this.loggerService.info(`[TelegramController] Received consultation request from ${consultationDto.email}.`);

			const result = await this.telegramService.sendConsultation(consultationDto);

			if (!result.success) {
				this.loggerService.warn(`[TelegramController] Failed to process consultation request for ${consultationDto.email}: ${result.message}`);
				return next(new HTTPError(502, result.message));
			}

			this.ok(res, result);
		} catch (error: any) {
			this.loggerService.error(`[TelegramController] Error handling consultation request: ${error.message}`);
			next(error);
		}
	}
}
