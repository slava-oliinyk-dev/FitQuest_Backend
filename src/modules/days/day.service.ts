...
export class DayService implements IDayService {
  ...
  async getDaysService(programId: number, userId: number): Promise<DayWithExercisesDto[]> {
    try {
      ...
      this.loggerService.info(
        `Retrieved ${days.length} days for program ${programId} and user ${userId}.`,
      );
  ...
  async createDayService(
    ...
    try {
      ...
      this.loggerService.info(
        `Day created with ID ${createdDay.id} in program ${programId} by user ${userId}.`,
      );
    ...
    try {
      ...
      if (!day) {
        this.loggerService.warn(
          `Day with ID ${dayId} not found for user ${userId} in program ${programId}.`,
        );
        throw new HTTPError(404, 'Day not found');
      }
  ...
  async deleteDayService(programId: number, dayId: number, userId: number): Promise<void> {
    try {
      ...
      this.loggerService.info(
        `Day with ID ${dayId} and all related records deleted in program ${programId} for user ${userId}.`,
      );
    } catch (error: any) {
      if (error instanceof HTTPError) {
        throw error;
