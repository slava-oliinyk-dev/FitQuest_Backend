...
export class ExerciseController extends BaseController implements IExerciseController {
    ...
    this.bindRoutes([
      ...
      },
      {
        ...
        middlewares: [
          ...
          new ValidateMiddleware(ExerciseDto),
        ],
      },
      {
      ...
      },
      {
        ...
        middlewares: [
          passport.authenticate('jwt', { session: false }),
          new ValidateMiddleware(UpdateExerciseNoteDto),
        ],
      },
      {
        path: '/:dayId/exercises/:exerciseId',
  ...
  async updateExerciseNoteController(
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
    try {
      ...
      const updatedExerciseNote = await this.exerciseService.updateExerciseNoteService(
        exerciseNoteDto,
        dayId,
        exerciseId,
        userId,
      ...
      this.loggerService.info(
        `User ${userId} updated exercise note with ID ${exerciseId} in day with ID ${dayId}.`,
      );
  ...
  async deleteExerciseController(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      this.loggerService.info(
        `User ${userId} deleted exercise with ID ${exerciseId} in day ${dayId}.`,
      );
