...
export class ExerciseController extends BaseController implements IExerciseController {
	...
	async updateExerciseNoteController(req: Request, res: Response, next: NextFunction): Promise<void> {
		try {
			...
			const updatedExerciseNote = await this.exerciseService.updateExerciseNoteService(exerciseNoteDto, dayId, exerciseId, userId);
	...
	async updateExerciseController(req: Request, res: Response, next: NextFunction): Promise<void> {
		try {
			...
			const updatedExercise = await this.exerciseService.updateExerciseService(exerciseDto, dayId, exerciseId, userId);
			this.loggerService.info(`User ${userId} updated exercise with ID ${exerciseId} in day with ID ${dayId}.`);
