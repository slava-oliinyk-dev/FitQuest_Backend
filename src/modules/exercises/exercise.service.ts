...
export class ExerciseService implements IExerciseService {
	...
	async getExercisesService(dayId: number, userId: number): Promise<ExerciseDto[]> {
		try {
			...
			if (exercises.length === 0) {
				...
				this.loggerService.info(`Retrieved ${exercises.length} exercises for day ${dayId} and user ${userId}.`);
			}
	...
	async createExerciseService(dto: ExerciseDto, dayId: number, userId: number): Promise<ExerciseDto> {
		try {
			const day = await this.dayRepository.findDayByIdAndUser(dayId, userId);
			if (!day) {
				this.loggerService.warn(`Day with ID ${dayId} not found for user ${userId}.`);
	...
	async updateExerciseService(dto: ExerciseDto, dayId: number, exerciseId: number, userId: number): Promise<ExerciseDto> {
		try {
			const day = await this.dayRepository.findDayByIdAndUser(dayId, userId);
			if (!day) {
				this.loggerService.warn(`Day with ID ${dayId} not found for user ${userId}.`);
	...
	async getExerciseService(dayId: number, exerciseId: number, userId: number): Promise<ExerciseDto> {
		try {
			const day = await this.dayRepository.findDayByIdAndUser(dayId, userId);
			if (!day) {
				this.loggerService.warn(`Day with ID ${dayId} not found for user ${userId}.`);
