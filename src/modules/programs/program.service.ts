...
export class ProgramService implements IProgramService {
  ...
  async createProgramService(dto: ProgramDto, userId: number): Promise<ProgramWithDaysDto> {
    try {
      ...
      return {
        ...
        creationDate: createdProgram.creationDate
          .toISOString()
          .split('T')[0]
          .split('-')
          .reverse()
  ...
  async updateProgramService(id: number, dto: ProgramDto, userId: number): Promise<ProgramDto> {
    try {
      ...
      const updatedProgram = await this.programRepository.updateProgramRepository(
        id,
        programEntity,
      );
  ...
  async deleteProgramService(
    ...
    try {
      ...
      const deletedProgram = await this.programRepository.deleteProgramRepository(
        programId,
        userId,
      );
      this.loggerService.info(
        `Program with ID ${programId} and all related records deleted by user ${userId}.`,
      );
